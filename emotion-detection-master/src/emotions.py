...
if mode == "train":
    ...
    img = cv2.imread('input_imgs/img0.jpg',1)
...
#         roi_gray = gray[y:y + h, x:x + w]
    ...
    output = cv2.putText(img, 'hahah...', org, font, fontScale, color, thickness, cv2.LINE_AA) 
...
#     while True:
#         # Find haar cascade to draw bounding box around f ace
#         ret, frame = cap.read()
#         if not ret:
#             break
#         facecasc = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
#         gray = cv2.cvtColor(frame, cv2.COLOR_BGR2acecasc = cv2.CascadeClassifier('haarcascadeGRAY)
#         faces = facecasc.detectMultiScale(gray,scaleFactor=1.3, minNeighbors=5)
